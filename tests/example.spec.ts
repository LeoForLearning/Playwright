...
test.beforeEach(async ({ page }) => {
  await page.goto('https://www.yougov-dev.chat/channels/qa-test-channelses/8307?recontact_id=7fff5d86-5371-4162-b6ff-350c8629ad65');
});
...
test.describe('messenger ', () => {
  test.only('should allow me to add todo items', async ({ page }) => {
    ...
    const user = await page.frameLocator('#ic-chat-widget-frame');
    await user.locator('"It is"').click();


    await user.locator('.message-text', { hasText: '📊 In a recent YouGov poll, 50% of Americans say the US is the greatest nation, while 31% say it is not.' }).isVisible();

    await user.locator('.button.button--quick-reply', { hasText: 'Begin 👉' }).click();
    await user.locator('.message-buttons', { hasText: 'Will' }).click();
    await expect(user.locator('.chart-wrapper.mt-2')).toBeVisible();
    await user.locator('.button.button--answer', { hasText: ' I am ' }).click();
